...
def ip_as_url(urls):
    ...
    for url in urls:
        if re.compile(IPREGEX, re.IGNORECASE).search(url) and re.compile(IPREGEX, re.IGNORECASE).search(url).group(1) is not None:
            result.append(re.compile(
                IPREGEX, re.IGNORECASE).search(url).group(1))
            count += 1
    return count
...
def get_features(path):
    ...
    feature['presence_flash_content'] = int(
        presence_flash_content(mail_body) == True)

    feature['presence_general_salutation'] = int(
        presence_general_salutation(mail_body) == True)

    feature['presence_java_script'] = int(
        presence_java_script(mail_body) == True)
    ...
    feature['presence_bad_ranked_url'] = int(
        presence_bad_ranked_url(urls) == True)
    ...
    feature['contains_prime_targets'] = int(
        contains_prime_targets(mail_subject) == True)

    feature['contains_suspended'] = int(
        contains_suspended(mail_subject) == True)
